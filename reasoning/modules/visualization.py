...
def add_text(text, fig=None, ax=None, **kwargs):
    if fig is None or ax is None:
        fig = plt.gcf()
        ax = fig.axes
    ax[0].text(0.2, 0.2, text, color='black', fontsize=30, ha='left', va='top', bbox=dict(facecolor='white', alpha=0.5, edgecolor='white', pad=10))
    return fig, ax
...
def plot_matches(mkpts0, mkpts1, fig=None, ax=None, color='b', alphas=None, **kwargs):
    if fig is None or ax is None:
        fig = plt.gcf()
        ax = fig.axes
    ...
    for i, c in enumerate(color):
        if isinstance(c, str) and c in default_colors:
    ...
    for i, (mkp0, mkp1) in enumerate(zip(mkpts0, mkpts1)):
        ...
        con = ConnectionPatch(
            ...
            color=color[i], linewidth=1.5, alpha=a, **kwargs)
