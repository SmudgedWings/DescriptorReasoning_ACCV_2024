...
if __name__ == "__main__":
    ...
    sample_image = load_image("assets/boat1.png").unsqueeze(0).repeat(4, 1, 1, 1)
    ...
    with torch.inference_mode():
        output = model({"image": sample_image.to(device)})
        keypoints = output["keypoints"]
        descriptors = output["descriptors"]
        
        output2 = model.forward_desc({"image": sample_image.to(device)}, keypoints)
        keypoints2 = output2["keypoints"]
        descriptors2 = output2["descriptors"]
        
    print(keypoints.shape, descriptors.shape) # B N 2, B N D
    print(keypoints2.shape, descriptors2.shape) # B N 2, B N D
    
    try:
        assert torch.allclose(keypoints, keypoints2, atol=1e-4)
        print("All tests passed!")
    except AssertionError:
        # find the difference
        for b in range(descriptors.shape[0]):
            print(f"Batch {b}")
            diff = torch.linalg.norm(descriptors[b] - descriptors2[b], dim=1)
            print("Difference in descriptors:")
            print(f"Max: {diff.max().item()}, Min: {diff.min().item()}")
            print("Indices of max difference:")
            print(diff.argmax())
