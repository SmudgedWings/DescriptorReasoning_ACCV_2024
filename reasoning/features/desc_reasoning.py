...
from reasoning.utils import Timer

import torch.nn.functional as F
import logging
import os 
from reasoning.datasets.utils import resize_long_edge
...
class ReasoningBase(torch.nn.Module):
    ...
    def forward(self, data, keypoints=None):
        ...
        '''
        with Timer("Forward", active=self.conf.activate_timers):
    ...
    def dual_softmax_loss(self, pred, data):
        from reasoning.datasets.scannet import get_correspondences_0to1, get_correspondences_1to0

        with Timer("DualSoftmaxLoss", active=self.conf.activate_timers):
            b_size = data['image0'].shape[0]
            
            if not self.conf.deep_supervision:
                desc_to_supervise0 = [pred["reasoning_features0"]]
                desc_to_supervise1 = [pred["reasoning_features1"]]
                semantic_to_supervise0 = [pred["semantic_features0"]]
                semantic_to_supervise1 = [pred["semantic_features1"]]
            else:
                desc_to_supervise0 = pred["all_reasoning_features0"]
                desc_to_supervise1 = pred["all_reasoning_features1"]
                semantic_to_supervise0 = pred["all_semantic_features0"]
                semantic_to_supervise1 = pred["all_semantic_features1"]
            ...
            for reasoning_features0, reasoning_features1, semantic_features0, semantic_features1 in zip(desc_to_supervise0, desc_to_supervise1, semantic_to_supervise0, semantic_to_supervise1):
                dist_mat = torch.einsum("bnd,bmd->bnm", reasoning_features0, reasoning_features1)
            
                if self.conf.semantic_conditioning:
                    sem_mat = torch.einsum("bnd,bmd->bnm", semantic_features0, semantic_features1)
                    dist_mat = dist_mat * sem_mat
            
                conf_matrix12 = F.log_softmax(dist_mat, dim=-1)
                conf_matrix21 = F.log_softmax(dist_mat.transpose(1, 2), dim=-1)
                ...
                batch_correspondences_0to1 = get_correspondences_0to1(keypoints0, data, -1, filter=False) # [B, N, 4]
                batch_correspondences_1to0 = get_correspondences_1to0(keypoints1, data, -1, filter=False) # [B, N, 4]
                
                # lets make some maks for keypoints that are visible in both images
                keypoints0_mask = batch_correspondences_0to1[..., 3] > -1 # check if the keypoint is visible in the second image
                keypoints1_mask = batch_correspondences_1to0[..., 0] > -1 # check if the keypoint is visible in the first image
                                
                for b in range(data['image0'].shape[0]):
                    correspondences = batch_correspondences_0to1[b]
                    
                    # calculate correspondences
                    correct_th = 3
                    distmat = torch.cdist(keypoints1[b], correspondences[:, 2:]) # [N, M]
                    m0_scores, m0 = distmat.min(dim=0) # [M]
                    valid = m0_scores <= correct_th # [M]
                    
                    match_idx0 = torch.where(valid, torch.arange(m0.shape[0], device=m0.device), -1)[valid] # [valid]
                    match_idx1 = m0[valid] # [valid]

                    # Now lets find all the keypoints from both images that has GT but no matches
                    should_not_match0 = keypoints0_mask[b].clone() # [N]
                    should_not_match1 = keypoints1_mask[b].clone() # [M]
                    
                    should_not_match0[match_idx0] = False
                    should_not_match1[match_idx1] = False
                    
                    # transform to indices
                    should_not_match0 = torch.where(should_not_match0)[0]
                    should_not_match1 = torch.where(should_not_match1)[0]

                    if valid.sum() > 0:
                        # First lets supervise the keypoints with correct gt correspondences
                        pred0 = conf_matrix12[b][match_idx0] # <- this needs to go up
                        pred1 = conf_matrix21[b][match_idx1] # <- this needs to go up
                        matchable_loss = matchable_loss + (F.nll_loss(pred0, match_idx1) + F.nll_loss(pred1, match_idx0)) / 2

                        if matchable_loss.isnan() or matchable_loss < 0:
                            logging.error("\n---------------------------")
                            logging.error(f"Matchable loss is NaN or negative")
                            logging.error(f"Matchable loss: {matchable_loss}")
                            logging.error("\n---------------------------")
                            import pdb; pdb.set_trace()
                            
                    losses['matchable'] += [matchable_loss / b_size]
                    losses['unmatchable'] += [unmatchable_loss / b_size]
